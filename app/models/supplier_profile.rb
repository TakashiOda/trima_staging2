class SupplierProfile < ApplicationRecord
  ...
  validates :manager_name, presence: true, length: { maximum: 30, message: "最大30文字まで" }
  validates :manager_name_kana, presence: true, length: { maximum: 30, message: "最大30文字まで" }
  validates :prefecture_id, inclusion: { in: 1..47 }
  validates :area_id, inclusion: { in: 1..11 }
  ...
  validates :post_code, presence: true, format: { with: POSTCODE_REGEX, message: "ハイフンあり半角数字のみ" }
  validates :phone, presence: true, format: { with: PHONE_NUMBER_REGEX, message: "ハイフンなし半角数字10桁または11桁のみ" }
